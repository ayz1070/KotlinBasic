...
// 변수의 접근을 위해 변수의 이름을 사용한다.
// 변수의 이름은 개발자가 정해주며 개발자는 변수의 이름을 통해 변수에 접근하여 변수에 값을 저장하거나
// 가져다 사용할 수 있다.

// 식별자
// 변수, 함수, 클래스, 메서드 등과 같이 개발자가 정의해서 사용하는 요소들을
// 구분하기 위해 지어주는 이름
// 식별자 규칙은 자바와 동일

// 문법적으로 지켜야 하는 규칙 (지키지 않으면 오류가 발생)
// 1. 길이의 제한은 없다.
// https://www.curioustore.com/ 변수명 생성해주는 사이트.. ㅎ
// 2. 첫 글자는 문자, _, $만 가능하다.
// 3. 두 번째 글자 부터는 문자, 숫자, _, $를 사용할 수 있다.
// 4. 자바에서 식별자를 한글로 지어주는 것이 허용되기 때문에 코틀린도 식별자는 한글로 해도 된다.

// 권장사항
// 1.변수의 이름은 첫 글자를 소문자로 한다.
// 2. 함수나 메서도의 이름은 첫 글자를 소문자로 한다.
// 3. 클래스의 이름은 첫 글자를 대문자로 한다.
// 4. 상수를 의미하는 것들은 모두 대문자로 한다.

// 표기 기법
// 1. 잘못된 표기 예 : koreanpointaverage (가독성이 떨어짐)
// 2. 파스칼 표기 기법
// 단어와 단어 사이에 언더바(_)를 두는 표기 기법
// - 수정 : korean_point_average
// 3. 카멜 표기 기법
...
fun main() {
    ...
    var a1:Int
    a1 = 100
    // 변수의 이름만 작성하면 변수에 저장되어 있는 값을 가져와 사용할 수 있다.
    println("a1 : $a1")
    ...
    // 자료형이 결정된 변수는 자료형을 변경할 수 없다.(형변환으로 가능하다.)
    // a4 = "안녕하세요"
    ...
    // 문자열 결합
    // 문자열 + 문자열 : 문자열 들을 합친 새로운 문자열이 생성된다.
